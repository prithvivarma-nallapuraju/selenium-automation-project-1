...
const shippingInformation = async (brwsr, folder, driver) => {
    ...
    try{
        ...
        const radioButton = await driver.findElement(By.id('billing:use_for_shipping_yes'));
        const isSelected = await radioButton.isSelected();

        if(isSelected){
            const continueElement1 = await driver.wait(until.elementLocated(By.xpath("(//button[@onclick='billing.save()'])[1]")))
            await driver.wait(until.elementIsVisible(continueElement1))
            await continueElement1.click()
